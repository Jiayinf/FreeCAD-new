...
private:
    ...
    // 同一轴方向累积的平移向量（世界坐标系下）
    SbVec3f transDelta;
    SbVec3f m_lastTranslationDelta;

    //// 累积的平移向量（世界坐标系下）
    //SbVec3f m_accumTranslationWorld;
    // 未设置位移方向：-1，沿x位移：0，沿y位移：1，沿z位移：2
    int m_lastTranslationAxis = -1;
    bool m_hasLastTranslationAxis = false;


    //仅为debug使用，标记start和current两个点
